...
package io.github.sadellie.indexxo.feature.similarimages
...
data object SimilarImagesScreen : Screen {
  private fun readResolve(): Any = SimilarImagesScreen

  @Composable
  override fun Content() {
    val localNavigator = LocalNavigator.currentOrThrow
    val screenModel = koinScreenModel<SimilarImagesScreenModel>()
    when (val uiState = screenModel.uiState.collectAsState().value) {
      is SimilarImagesUIState -> IndexedObjectsGroupSelectorView(
        uiState = uiState,
...
data class SimilarImagesUIState(
...
class SimilarImagesScreenModel(
  ...
  override fun onDiscardSelected() {
    screenModelScope.launch {
      indexedObjectRepository.discardSimilarImages(selectedPaths.value)
    }
  }
}
