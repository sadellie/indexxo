...
private abstract class ReflectLayoutHitTestOwner : LayoutHitTestOwner {
  ...
  private val ownerRootField =
    classLoader.loadClass("androidx.compose.ui.node.Owner").getDeclaredMethod("getRoot").apply {
      trySetAccessible()
    }
  ...
  private val layoutNodeHitTestMethod =
    classLoader.loadClass("androidx.compose.ui.node.LayoutNode").declaredMethods.first {
      it.name.startsWith("hitTest-") && it.parameterCount == 4
    }
  ...
  protected class CopiedList<T>(private val populate: (MutableList<T>) -> Unit) :
...
@OptIn(InternalComposeUiApi::class)
private class SingleLayerLayoutHitTestOwner(scene: ComposeScene) : ReflectLayoutHitTestOwner() {
...
@OptIn(InternalComposeUiApi::class)
private class MultiLayerLayoutHitTestOwner(private val scene: ComposeScene) :
  ReflectLayoutHitTestOwner() {

  private val sceneClass =
    classLoader.loadClass("androidx.compose.ui.scene.MultiLayerComposeSceneImpl")
  private val layerClass =
    sceneClass.declaredClasses.first {
      it.name == "androidx.compose.ui.scene.MultiLayerComposeSceneImpl\$AttachedComposeSceneLayer"
    }
