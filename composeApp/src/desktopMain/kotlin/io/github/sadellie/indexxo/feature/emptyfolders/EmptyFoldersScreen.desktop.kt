...
package io.github.sadellie.indexxo.feature.emptyfolders
...
data object EmptyFoldersScreen : Screen {
  private fun readResolve(): Any = EmptyFoldersScreen

  @Composable
  override fun Content() {
    val localNavigator = LocalNavigator.currentOrThrow
    val screenModel = koinScreenModel<EmptyFoldersScreenModel>()
    when (val uiState = screenModel.uiState.collectAsState().value) {
      is IndexedObjectsFlatSelectorUIState -> IndexedObjectsFlatSelectorView(
        uiState = uiState,
        title = stringResource(Res.string.empty_folders),
...
class EmptyFoldersScreenModel(
  ...
  override val uiState: StateFlow<IndexedObjectsFlatSelectorUIState?> = combine(
    indexedObjectRepository.emptyFolders,
    selectedPaths,
    moveToTrashProgress,
